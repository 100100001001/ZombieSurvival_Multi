...
public class ZombieSpawner : MonoBehaviourPun, IPunObservable
{
    ...
    private void Awake()
    {
        PhotonPeer.RegisterType(typeof(Color), 127, ColorSerialization.SerializeColor, ColorSerialization.DeserializeColor);
    }
    ...
    // 포톤의 Network.Destroy()는 지연 파괴를 지원하지 않으므로 지연 파괴롤 직접 구현함
